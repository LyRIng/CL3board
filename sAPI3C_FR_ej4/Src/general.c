...
// Adapted 05.2022 for Final Display of WDIR--
void WDir_Screen(char *task_msg, int16_t wdir){   
    ...
    ILI9341_WriteString(10, 6*10, "METEO Readings - CL3", Font_11x18, ILI9341_GREEN, ILI9341_BLACK);
    ILI9341_WriteString(10, 4*10+4*18, "F1-WindDirect", Font_16x26, ILI9341_BLUE, ILI9341_BLACK);
...
void QueueAndSemCreateAll (void){
  ...
  if((packet_queue != NULL) && (output_write_queue != NULL) && (command_queue != NULL) && (xMutex != NULL) && xSem1 != NULL )
    ...
    {
	  sprintf(usr_msg,"Failed queue or mutex creation!!\r\n");
	  printmsg(usr_msg);
    }
}
